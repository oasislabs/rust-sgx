...
pub fn https_get(url: &str,
    ...
    let mut response = client.get(url).send().map_err(|e| format!("Failed downloading, error: {:?}, url: {}", e, url))?;
...
pub fn https_put(url: &str,
    ...
    let connector = HttpsConnector::new(ssl);
    let client = hyper::Client::with_connector(Pool::with_connector(Default::default(), connector));
    let result = client.put(url).body(body.as_slice()).send().map_err(|e| format!("Failed upload, error: {:?}, url: {}", e, url))?;
...
pub fn encrypt_buffer(body: &[u8], encryption: &CredentialsEncryption) -> Result<Vec<u8>, String>{
    ...
    Rdrand.random(&mut nonce[..]).map_err(|e| format!("Could not generate andom nonce {}", e))?;
