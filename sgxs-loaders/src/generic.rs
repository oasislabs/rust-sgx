...
pub(crate) trait EnclaveLoad: Debug + Sized + Send + Sync + 'static {
    ...
    fn create_controller(mapping: &Mapping<Self>) -> Self::EnclaveController;
}
...
impl<C: 'static + Debug + Clone, T: loader::Load<MappingInfo = MappingInfo<C>, Tcs = Tcs> + ?Sized> Into<loader::Mapping<T>>
    for LoadResult<C>
{
...
impl<D: EnclaveLoad> Device<D> {
    pub fn load(
        ...
        let enclave_controller = Some(D::create_controller(&mapping));
